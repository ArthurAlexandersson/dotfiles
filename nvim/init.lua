...
require("lazy").setup({
	...
	--    }
  {
		...
		config = function()
			require("themery").setup({
				themes = {
					...
					{ name = "Rose Pine Moon", colorscheme = "rose-pine" },
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			vim.keymap.set(
				...
				{ desc = "[S]earch: [R]esume last search" }
			)
		...
		opts = {
			formatters = {
				uncrustify = {
					command = "uncrustify",
					args = function()
						local cfg = vim.fn.stdpath("config") .. "/uncrustify.cfg"
						if vim.fn.filereadable(cfg) == 1 then
							return { "-c", cfg, "-f", "$FILENAME" }
						else
							return { "-f", "$FILENAME" }
						end
					end,
...

	{
		...
		priority = 1000,
		config = function()
			require("ayu").setup({ mirage = true }) -- true = dark
...

	{
		...
		config = function()
			require("catppuccin").setup({
				flavour = "mocha", -- latte = light, mocha = dark
			})
...

	{
		...
		priority = 1000,
		config = function()
			vim.cmd.colorscheme("kanagawa-wave") -- or kanagawa-dragon, kanagawa-lotus (light)
		end,
	},

	{
		...
		config = function()
			require("rose-pine").setup({
				variant = "moon", -- dawn = light, moon/main = dark
			})
...

	{
		...
		config = function()
			...
			vim.cmd.colorscheme("nightfox") -- or carbonfox, nordfox
		end,
	},
	...
	--
	require("nvimplugins.plugins.debug"),
	require("nvimplugins.plugins.lint"),
	require("nvimplugins.plugins.indent_line"),
	require("nvimplugins.plugins.autopairs"),
	require("nvimplugins.plugins.neo-tree"),
	require("nvimplugins.plugins.gitsigns"), -- adds gitsigns recommend keymaps

	-- Open Neo-tree filesystem view
	vim.api.nvim_set_keymap("n", "<leader>nn", ":Neotree toggle filesystem<CR>", { noremap = true, silent = true }),
